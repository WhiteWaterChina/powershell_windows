...
(Get-WmiObject win32_diskdrive|where {$_.status -eq "OK"}|Sort-Object -Property deviceid |Select-Object -ExpandProperty serialnumber)|Out-File -Append -Force -Encoding unicode "$log_name"
...
Write-Host "Below is the Network Speed(bit)!" -ForegroundColor Green
...
echo "Below is the Network Speed(bit)!"|Out-File -Append -Force -Encoding unicode "$log_name"
(Get-WmiObject win32_networkadapter|where {$_.physicaladapter -eq "True"}|Sort-Object -Property deviceid|where {if($_.netenabled -eq "True"){echo ($_.speed)|Out-File -Append -Force -Encoding unicode "$log_name"}else{echo "None(Not Enabled!)"|Out-File -Append -Force -Encoding unicode "$log_name"}})
...
Get-WmiObject win32_networkadapter|where {$_.physicaladapter -eq "True"}|
Sort-Object -Property deviceid|Select-Object -ExpandProperty speed|
Out-File -Force -Append "$log_dir\temp_net_speed.log"
Get-WmiObject win32_networkadapter|where {$_.physicaladapter -eq "True"}|
Sort-Object -Property deviceid|Select-Object -ExpandProperty guid|
...
where {if($_.netenabled -eq "True"){echo ($_.speed)|Out-File -Append -Force -Encoding unicode "$log_name"}else{echo "None(Not Enabled!)"|
Out-File -Append -Force  -Encoding unicode "$log_name"}})
echo "Below is the Network Manufacturer!" |Out-File -Append -Force -Encoding unicode "$log_name"
(Get-WmiObject win32_networkadapter|where {$_.physicaladapter -eq "True"}|
Sort-Object -Property deviceid|
...
Select-Object -ExpandProperty hash
$hash_base_net_speed=Get-FileHash -Path $log_dir\base_net_speed.log -Algorithm SHA256|
Select-Object -ExpandProperty hash
$hash_base_net_guid=Get-FileHash -Path $log_dir\base_net_speed.log -Algorithm SHA256|
Select-Object -ExpandProperty hash
...
Select-Object -ExpandProperty hash
$hash_temp_net_speed=Get-FileHash -Path $log_dir\temp_net_speed.log -Algorithm SHA256|
Select-Object -ExpandProperty hash
$hash_temp_net_guid=Get-FileHash -Path $log_dir\temp_net_speed.log -Algorithm SHA256|
Select-Object -ExpandProperty hash
...
{
echo "Network Speed check OK!"|Out-File -Append -Force -Encoding unicode "$log_name"
echo "OK" |Out-File -Append -Force "$log_dir\status.log"
}
else
{
echo "Network Speed check FAIL!"|Out-File -Append -Force -Encoding unicode "$log_name"
echo "FAIL" |Out-File -Append -Force "$log_dir\status.log"
}
...
{
echo "Network GUID check OK!"|Out-File -Append -Force -Encoding unicode "$log_name"
echo "OK" |Out-File -Append -Force "$log_dir\status.log"
}
else
{
echo "Network GUID check FAIL!"|Out-File -Append -Force -Encoding unicode "$log_name"
echo "FAIL" |Out-File -Append -Force "$log_dir\status.log"
}
...
$log_name="${current_path}\log\${current_date}_SIT_REBOOT_TEST_WIN.log"
#echo $log_dir
